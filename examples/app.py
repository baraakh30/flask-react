...
def user_detail(user_id):
    ...
    if not user:
        return (
            react.render_template(
                "NotFound", message=f"User with ID {user_id} not found"
            ),
            404,
        )
...
def product_list():
    ...
    if search:
        filtered_products = [
            p for p in filtered_products if search.lower() in p["name"].lower()
        ]
